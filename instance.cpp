	...
	for(i=0;i<numAttributes;i++) {
		if(missing[i]) {
			if(ai.getTypeOfAttribute(i)==REAL) {
				float val=ai.getAverageOfAttribute(i);
				realValues[i]=val;
				//printf("Value for attribute %d %g\n",i,val);
			} else if(ai.getTypeOfAttribute(i)==NOMINAL) {
				int val=ai.getMostFrequentValueOfAttribute(i);
				//nominalValues[i]=val;
				realValues[i]=val;
...
float instance::distance(instance *other)
{
	float dist = 0;
	int i;
	...
	for(i=0;i<numAttributes;i++) {
		if(ai.getTypeOfAttribute(i)==NOMINAL) {
			if(realValues[i]!=other->realValues[i]) dist++;
		} else {
			register float diff = (realValues[i]-other->realValues[i])/ai.getSizeDomain(i);
			dist+=diff * diff;
		}
	}
	/*int num = ai.getNumRealAttributes();
	int *atts = ai.getRealAttributes();
	for(i=0;i<num;i++) {
		register int att = atts[i];
		dist+=fabs(realValues[att]-other->realValues[att])/ai.getSizeDomain(att);
	}
	num = ai.getNumNominalAttributes();
	atts = ai.getNominalAttributes();
	for(i=0;i<num;i++) {
		register int att = atts[i];
		if(realValues[att]!=other->realValues[att]) dist++;
	}*/
