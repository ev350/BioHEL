...
timerMDL::timerMDL()
{
	...
	int i;
	coverageBreak=cm.getParameter(COVERAGE_BREAKPOINT);
	...
	for(i=0;i<nc;i++) {
		...
		coverageBreaks[i]=coverageBreak/(double)ai.getInstancesOfClass(i)*(double)is->getNumInstances();
...
void timerMDL::newIteration(int iteration, int lastIteration)
{
	...
	if (iteration == startIteration) {
		...
		if(!fixedWeight) {
			...
			if(error==0) {
				...
				mdlWeight =
				    (initialTheoryLenghtRatio /
				     (1 - initialTheoryLenghtRatio))
				    * (error / theoryLength);
			}
		}
		updateWeight = 1;
	...
	if (activated && !fixedWeight) {
		if (pw->getBestPopulation()->getExceptionsLength() != 0) {
			if (tEvolStats->getIterationsSinceBest()
			    == 10) {
	...
	if (updateWeight) {
		...
		mb.printf("MDL Theory Length Weight: %.10f (%d)\n", mdlWeight,tEvolStats->getGlobalIterationsSinceBest());
		...
		//} else {
			if(coverageBreaks[cl]<1) {
				if(cov<coverageBreaks[cl]) {
					...
					if(cov>coverageBreaks[cl]*2) cov=coverageBreaks[cl]*2;
