...
int isMajority(classifier & ind)
{
	...
	int posMax=0;
	int tie=0;
	//printf("Instances of class 0: %d\n",classCounts[0]);
	for(i=1;i<nc;i++) {
		//printf("Instances of class %d: %d\n",i,classCounts[i]);
...
double classifierFitness(classifier & ind)
{
	...
	for (i = 0; i < numInstances; i++) {
		if(ind.doMatch(instances[i])) {
			//mb.printf("Inst %d %d %d\n",i,cl,instances[i]->instanceClass);
...
void classifierFitness2(classifier & ind)
{
	int i;
	int numInstances = is->getNumInstancesOfIteration();
	int cl=ind.getClass();
