...
float classifier_hyperrect_list_ratio::mutationOffset(float geneValue, float offsetMin,
                                                      float offsetMax)
{
    float newValue;
    if (!rnd < 0.5) {
...
void classifier_hyperrect_list_ratio::dumpPhenotype(char *string)
{
    ...
    for (i = 0,index=0; i < numAtt; i++) {
        ...
        sprintf(temp,"Att %s / Att %s is " ,ai.getAttributeName(attIndex)->cstr(), ai.getAttributeName(attIndex+1)->cstr());
        int irr=1;
                                                    ...
                                                    classifier_hyperrect_list_ratio * out2)
{
    ...
    if(in1->numAtt==0) {
        classifier_hyperrect_list_ratio *tmp=in2;
        in2=in1;
        in1=tmp;
    }
