...
timerRealKR::timerRealKR()
{
	...
	fixExpAtt = cm.thereIsParameter(FIX_EXPRESSED_ATTS_INIT);
	if(!fixExpAtt) {
		probIrr= 1-(double)numExpAtt/(double)numAtt;
		mb.printf("Probability of irrelevant attribute set to %f\n",probIrr);
	}

	minIntervalSizeInit=cm.getParameter(MIN_INTERVAL_SIZE_INIT);
	rangeIntervalSizeInit=cm.getParameter(MAX_INTERVAL_SIZE_INIT)-minIntervalSizeInit;
	...
	if(hyperrectList) {
		...
		sampAtts = new Sampling(ai.getNumAttributesMC());
